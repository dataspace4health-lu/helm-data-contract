...
import registerDataAsset from "./register.json";
...
test.describe("Data Contract Non Negotiable Flow", () => {
  ...
  test("Make contract", async ({ request, baseURL }) => {
    ...
    // Add the consumer signature to the proof array
    makeContractBody.verifiableCredential[0].proof.push(consumerSignature);
    makeContractBody.verifiableCredential[0].credentialSubject["gax:consumer"] =
      consumerDid;
